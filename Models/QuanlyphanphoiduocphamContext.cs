...
namespace PharmaDistributionApp.Models
{
    public partial class QuanlyphanphoiduocphamContext : DbContext
    {
        ...
        {
            // 1. CẤU HÌNH CÁC BẢNG CÓ KHÓA TỔ HỢP
            ...
            modelBuilder.Entity<Hoadonnhap>(entity =>
            {
                ...
                entity.Property(e => e.Sohdnhap).HasColumnName("SOHDNHAP");
                entity.HasOne<Nhacungcap>().WithMany().HasForeignKey(d => d.Mancc);
            });
            ...
            modelBuilder.Entity<Hoadonxuat>(entity =>
            {
                ...
                entity.Property(e => e.Sohdxuat).HasColumnName("SOHDXUAT");
                entity.HasOne<Khachhang>().WithMany().HasForeignKey(d => d.Makh);
            });
            ...
            modelBuilder.Entity<Sanpham>(entity =>
            {
                ...
                // --- ĐOẠN SỬA QUAN TRỌNG ---
                entity.HasOne<Loaisp>()          // Sanpham có 1 Loaisp (nếu trong Sanpham có property Loaisp thì dùng x => x.Loaisp)
                      .WithMany(l => l.Sanphams) // <--- QUAN TRỌNG: Phải trỏ vào list Sanphams bên Loaisp.cs
                      .HasForeignKey(x => x.Maloai)
                      .HasConstraintName("FK_SANPHAM_LOAISP"); // (Tùy chọn) Đặt tên cho khóa ngoại trong SQL
            });

            modelBuilder.Entity<Kho>(e => { e.ToTable("KHO"); e.HasKey(x => x.Makho); e.Property(x => x.Makho).HasColumnName("MAKHO"); });

            // --- SỬA CẤU HÌNH LOHANG (THÊM CỘT NHACUNGCAP) ---
            modelBuilder.Entity<Lohang>(e => {
                e.ToTable("LOHANG");
                e.HasKey(x => x.Malo);
                e.Property(x => x.Malo).HasColumnName("MALO");
                e.Property(x => x.Nhacungcap).HasColumnName("NHACUNGCAP"); // Đã thêm dòng này
            });

            modelBuilder.Entity<Loaisp>(entity =>
            {
                entity.ToTable("LOAISP");
                entity.HasKey(e => e.Maloai); // Khai báo khóa chính
